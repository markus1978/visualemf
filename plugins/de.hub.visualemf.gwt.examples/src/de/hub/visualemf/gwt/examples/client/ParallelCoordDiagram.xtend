...
class ParallelCoordDiagram extends FlowPanel {
	...
	val margin_right = 10
	val margin_left = 10
	val margin_bottom = 10
	
	val width = 960 - margin_left - margin_right
	val height = 300 - margin_top - margin_bottom
	
	val x = D3.scale.ordinal.rangePoints(0, width, 1)
	val dimensionIndexes = {}
	val y = {}
	val dragging = {}
	
	val line = D3.svg.line
	val axis = D3.svg.axis.orient(Axis.Orientation::LEFT)
	
	def render() {
		val svg = D3.select(this)
			.append("svg")
			.attr("width", width + margin_left + margin_right)
	        .attr("height", height + margin_top + margin_bottom + 100)
	        .append("g")
