...
class ChordDiagram extends FlowPanel {
	...
	def DatumFunction<Void> fade(ChordDiagramStylesCssResource css, Selection svg, double opacity) {
	...
	private static class GroupTick extends JavaScriptObject {
		...
		@JsNative static def GroupTick create(double angle, String label) '''
			return {
				angle : angle,
				label : label
			};
